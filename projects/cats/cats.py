...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def comparison(paragraphs):
        for word in topic:
            for item in [remove_punctuation(last) for last in [lower(word) for word in split(paragraphs)]]:
                if word == item:
                    return True
        return False
    return comparison
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 and len(reference_words) == 0:
        ...
        word = 0
        correct = 0
        while word < min(len(typed_words), len(reference_words)):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if typed_word == word:
            return typed_word
        comparison_list.append(diff_function(typed_word, word, limit))
    if min(comparison_list) > limit:
        return typed_word
    elif min(comparison_list) <= limit:
        return word_list[comparison_list.index(min(comparison_list))]
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == '' or goal == '':  # Fill in the condition
        ...
        compare = minimum_mewtations(start[1:] , goal[1:], limit)
        return compare
    
        # BEGIN

    return min(add, remove, substitute)
        #return the minimum of add, remove, subs
        # END
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    progress_ratio = number_correct / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        player_time = []
        for time in range(len(player) - 1):
            data_saved = player[time + 1] - player[time]
            player_time.append(data_saved)
        time_difference.append(player_time)
    return match(words, time_difference)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    fastest_player = []
    fastest_player = [[] for player in player_indices] 
    for word in word_indices:
        min_time = (float('inf'))
        new_player = 0
        for player in player_indices:
            if time(match, player, word):
                if min_time > time(match, player, word):
                    min_time = time(match, player, word)
                    new_player = player
        fastest_player[new_player].append(word_at(match, word))
