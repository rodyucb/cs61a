...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        score_holder = player_score
        while opponent_score > 0:
            digit = opponent_score % 10
            score_holder = digit_fn(digit)(score_holder)
            opponent_score = opponent_score // 10
        return score_holder % 30
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 = score0 + take_turn(strategy, score0, score1, dice, goal)
            score0 = score0 + hog_pile(score0, score1)
        else:
            strategy = strategy1(score1, score0)
            score1 = score1 + take_turn(strategy, score1, score0, dice, goal)
...
def make_averaged(original_function, total_samples=1000):
    ...
    def average(*args):
        ...
        for x in range(total_samples):
            sample_of_data = original_function(*args)
            sum = sum + sample_of_data
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_of_dice in range(1, 11):
        calculated_score_achieved = make_averaged(roll_dice, total_samples)(num_of_dice, dice)
        if calculated_score_achieved > best_score_achieved:
            best_score_achieved = calculated_score_achieved
            max_best_num_of_dice = num_of_dice
        num_of_dice = num_of_dice + 1
    return max_best_num_of_dice
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    #we need to see what the score will be if we roll zero dice
    #if hog_pile applies to that score, check if the points will be added
    #return 0

    #change/ or modify the else statement
    #return hefty-hogs_strategy
    if hefty_hogs_strategy(score, opponent_score, threshold, num_rolls) == 0:
        return 0
    else:
        score = score + hefty_hogs(score, opponent_score)
